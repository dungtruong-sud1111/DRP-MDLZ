...
**Mục tiêu giải pháp:** Module được thiết kế để giải quyết các điểm rời rạc của hiện trạng, hướng tới các mục tiêu:
...
**Mục đích:** Trước đây ba khâu phân bổ – tích hợp – theo dõi được làm rời rạc bằng Excel và Zalo, mỗi khâu một công cụ, dữ liệu không nối nhau nên khó kiểm soát và số liệu tiến độ dễ lệch. Giải pháp tổ chức ba khâu này thành **một chuỗi xử lý liền mạch** trên cùng một module, dữ liệu chảy thẳng từ kế hoạch đầu vào đến tiến độ đầu ra.
...
**Bước 1 — Lấy thực tế.** Hệ thống định kỳ lấy dữ liệu thực thi từ WMS theo mã đơn dùng chung.

**Bước 2 — Đối soát.** Hệ thống so sánh thực tế với kế hoạch để tính Complete / Pending / phần trăm.

**Bước 3 — Tách Stage Transfer.** Hệ thống tách hàng đã xuất nhưng còn ở khu vực trung chuyển (Stage Transfer) khỏi hàng đã thực sự đưa lên kệ, tránh số liệu "ảo".

**Bước 4 — Hiển thị & báo cáo.** Hệ thống trình bày tiến độ trên dashboard và cho xuất/chia sẻ báo cáo theo luồng/ngày kế hoạch.
...
**Mục đích:** Kế hoạch chuyển kho và tồn kho được nói bằng nhiều đơn vị khác nhau — kế hoạch theo **pallet/case**, còn tồn khả dụng WMS trả về theo **đơn vị nhỏ nhất (PCS)**. Để Module quy đổi nhất quán giữa các đơn vị (PCS ↔ CASE ↔ PALLET) khi phân bổ và khi hiển thị tiến độ, mỗi mặt hàng cần một bộ **hệ số quy đổi đơn vị** chuẩn. Trước đây việc quy đổi này nằm trong công thức Excel của từng người; mục này chuẩn hóa thành master data dùng chung.
...
**Mục đích:** Tham số hóa các ngưỡng và quy tắc vận hành (ngưỡng tách đơn, cơ chế kiểm soát hạn dùng, bước duyệt) thay vì cố định trong mã nguồn, để khi chính sách kho thay đổi thì người quản trị tự điều chỉnh được mà không cần can thiệp kỹ thuật. *(Việc gán vai trò thực hiện từng bước thuộc mục **7. Phân quyền hệ thống**.)*
...
**Mục đích:** Lâu nay quy tắc lấy hàng — kho nào lấy trước, kho nào gộp chung — nằm trong kinh nghiệm của đội kho: quen việc thì biết "ưu tiên BKD2 và BKD3, thiếu mới lấy thêm BKD1". Cách làm này phụ thuộc vào người, nên đổi ca hay có nhân sự mới là dễ chọn sai nguồn. Mục này đưa quy tắc đó ra thành một bảng cấu hình để hệ thống tự chọn đúng nguồn lấy hàng cho từng luồng; sau này muốn đổi quy tắc (thêm kho, đổi thứ tự dự phòng) thì chỉ sửa ở bảng này, không phải sửa phần mềm và không phải suy đoán từ cấu hình kho.
...
Hiện nay Planner MDLZ gửi kế hoạch qua mail/file, đội kho mở đọc và đối chiếu thủ công — qua nhiều khâu trung gian, dễ bỏ sót dòng, sai cột, không có bước kiểm tra trước khi đưa vào tính toán. Vì hệ thống DRP (tự động tính lượng điều chuyển) chưa triển khai, đầu vào của module hiện là **file Excel kế hoạch do Planner MDLZ tải lên**.
...
**Mục đích:** Hiện trạng, sau khi chốt lô trên Excel, nhân viên kho nhập tay từng đơn vào WMS và tự "xé" đơn cho vừa giới hạn vận hành — vừa tốn công nhập lại, vừa dễ tách sai (gộp nhầm hàng của hai kho nguồn vào một đơn, hoặc một đơn quá lớn khiến tồn không kịp lên SAP). Nghiệp vụ này để hệ thống **tự sinh đơn TO** từ phương án đã chốt theo quy tắc tách tường minh, không nhập lại tay; mỗi đơn mang **mã SO dùng chung** làm khóa nối Plan ↔ TO ↔ kết quả WMS, phục vụ tích hợp (mục **5.2**) và đối soát tiến độ (mục **5.3**).
...
**Mục đích:** Hiện trạng, Planner MDLZ / nhân viên kho gom đơn và chọn vị trí lấy hàng theo kinh nghiệm thủ công — dễ tạo lộ trình đi lại lòng vòng, một SKU nằm nhiều vị trí thì chọn cảm tính, không nhất quán. Nghiệp vụ này để hệ thống **tự gom SKU gần nhau và chỉ định vị trí lấy hàng tối ưu**, cho ra đơn TO có lộ trình soạn liền mạch và kết quả lặp lại được.
...
Hiện trạng, sau khi chốt lô trên Excel, nhân viên kho **nhập tay từng đơn vào WMS** — tốn công gõ lại, dễ sai trường, dễ tạo trùng đơn khi thao tác lặp, và không truy được đơn nào ứng với dòng kế hoạch nào. Nghiệp vụ này để hệ thống **tự tích hợp đơn xuống WMS qua API**: ánh xạ đúng trường, chống trùng theo mã SO, tự thử lại khi lỗi, và **lưu liên kết Plan ↔ SO ↔ ORDER KEY** để mọi đơn đều truy ngược được tới kế hoạch và để khâu theo dõi tiến độ (mục **5.3**) lấy đúng kết quả thực thi theo từng đơn. Cả hai luồng dùng **chung một cơ chế tích hợp**, đi thẳng xuống WMS (không vòng qua SAP).
...
Hiện trạng, tiến độ chuyển kho được cập nhật bằng **trao đổi trực tiếp / nhắn Zalo**: kho làm tới đâu báo tới đó, kế hoạch nắm số liệu rời rạc và trễ, không ai có **toàn cảnh gần thời gian thực**. Nặng hơn, số "đã hoàn thành" hay bị **"ảo"** — đơn In-In được WMS *tự ghi nhận (Auto-Receive)* ngay khi kho nguồn bấm Xuất, nên nhìn trên hệ thống tưởng đã xong, nhưng hàng thực tế còn nằm ở **khu trung chuyển (Stage Transfer)** của BKD1, chưa lên kệ để bán được.

Nghiệp vụ này thay báo cáo thủ công bằng **một dashboard đối soát Plan vs Actual tự động**: hệ thống định kỳ lấy thực tế từ WMS theo khóa đơn dùng chung, so với kế hoạch, **tách phần đã thực sự lên kệ khỏi phần còn ở Stage Transfer** để số liệu phản ánh đúng thực tế, và cho xuất báo cáo theo luồng/ngày để chia sẻ. _(Truy vết: BR-021/022/023/024/025/026/027/028, BR-033; NFR-04; BRULE-09/10; thay PP-03 "Complete ảo" & PP-06 "báo cáo Zalo".)_
